...
export class AuthService {
  ...
  async register(registerData: RegisterRequestDto): Promise<ApiResponse> {
    try {
      ...
      const endpoint = userType === 'user' ? '/auth/register' : `/auth/${userType}/register`;
      
      console.log('🚀 Attempting registration at:', endpoint, 'with data:', { ...registerData, password: '[HIDDEN]' });
      
      const response = await api.post<any>(endpoint, registerData);
      
      console.log('📥 Registration response:', response);
      
      // Check if response is an error message (string) indicating existing email
      if (typeof response === 'string' && 
          (response.includes('already registered') || 
           response.includes('already exists') ||
           response.includes('use a different email'))) {
        // This is actually an error response, throw it
        console.log('❌ Registration failed - email already exists:', response);
      ...
      const errorMessage = error.message || error.response?.data?.message || error.response?.data || 'Registration failed';
      throw new Error(errorMessage);
    }
  }
