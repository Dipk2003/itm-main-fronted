...
const Register: React.FC<RegisterProps> = ({ 
  ...
  // Determine redirect URL based on userType if not explicitly provided
  ...
  const finalRedirectTo = getRedirectUrl();
  ...
  React.useEffect(() => {
    if (isAuthenticated) {
      console.log('🎉 User authenticated, redirecting to:', finalRedirectTo);
      router.push(finalRedirectTo);
    }
  }, [isAuthenticated, router, finalRedirectTo]);

  React.useEffect(() => {
    // Clear form data when OTP is sent to prevent resubmission
    if (otpSent) {
      console.log('📧 OTP sent, clearing form data');
    }
  }, [otpSent]);
