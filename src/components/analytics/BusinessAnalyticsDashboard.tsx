...
const BusinessAnalyticsDashboard: React.FC = () => {
  ...
  const fetchAnalyticsData = useCallback(async () => {
    try {
      ...
      const [metricsResponse, timeSeriesResponse, regionalResponse] = await Promise.all([
        fetch(`${process.env.NEXT_PUBLIC_API_BASE_URL}/api/analytics/business-metrics`, {
          headers: { 'Authorization': `Bearer ${token}` }
        }),
        fetch(`${process.env.NEXT_PUBLIC_API_BASE_URL}/api/analytics/time-series?timeframe=${selectedTimeframe}`, {
          headers: { 'Authorization': `Bearer ${token}` }
        }),
        fetch(`${process.env.NEXT_PUBLIC_API_BASE_URL}/api/analytics/regional-data`, {
          headers: { 'Authorization': `Bearer ${token}` }
        })
      ]);

      if (!metricsResponse.ok || !timeSeriesResponse.ok || !regionalResponse.ok) {
