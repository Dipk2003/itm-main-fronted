...
const SmartRecommendations: React.FC<SmartRecommendationsProps> = ({
  ...
  const loadRecommendations = useCallback(async () => {
    ...
    try {
      ...
      if (type === 'products' || type === 'all') {
        if (actualUserId) {
          promises.push(
            aiRecommendationService.getPersonalizedRecommendations(String(actualUserId), {
              count: maxItems,
        ...
        if (actualUserId && user?.role) {
          promises.push(
            aiRecommendationService.getBusinessInsights(
              String(actualUserId),
              (user.role === 'ROLE_VENDOR' || user.role === 'SELLER') ? 'vendor' : 'buyer'
            )
          );
        }
      }
      ...
      if (type === 'pricing' || type === 'all') {
        if ((user?.role === 'ROLE_VENDOR' || user?.role === 'SELLER') && actualUserId) {
          promises.push(
            aiRecommendationService.getPriceOptimization(String(actualUserId), productId)
          );
        }
      }
      ...
      if (actualUserId && newRecommendations.products.length > 0) {
        aiRecommendationService.trackUserBehavior(String(actualUserId), {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Price Optimization */}
      {recommendations.pricing.length > 0 && (user?.role === 'ROLE_VENDOR' || user?.role === 'SELLER') && (
        <div>
          <div className="flex items-center justify-between mb-4">
            <h3 className="text-lg font-semibold text-gray-900 flex items-center">
