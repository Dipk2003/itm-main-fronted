...
import { registerBuyer, verifyOTP, clearError, setTempCredentials } from '@/features/auth/authSlice';
...
export default function UserRegisterPage() {
  ...
  const { loading, error, otpSent, isAuthenticated, user, registrationStep } = useSelector((state: RootState) => state.auth);
  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    // FIXED: Use correct buyer registration data structure
    const buyerRegistrationData = {
      name: formData.name,
      email: formData.email,
      phone: formData.phone,
      password: formData.password,
      aadharCard: formData.aadharCard, // Include Aadhar card for buyers
    };

    const result = await dispatch(registerBuyer(buyerRegistrationData));
  ...
  const handleOtpVerification = async (e: React.FormEvent) => {
    ...
    if (verifyOTP.fulfilled.match(result)) {
      console.log('✅ OTP verified successfully, should redirect now');
  ...
  useEffect(() => {
    if (isAuthenticated && user && registrationStep === 'completed') {
      console.log('✅ User authenticated after registration, redirecting to dashboard');
      console.log('User details:', { role: user.role, userType: user.userType });
      
      // Determine redirect path based on user role
      let redirectPath = '/dashboard/user'; // Default for buyers
      
      if (user.role === 'ROLE_USER' || user.userType === 'user') {
        redirectPath = '/dashboard/user';
      } else if (user.role === 'ROLE_VENDOR' || user.userType === 'vendor') {
        redirectPath = '/dashboard/vendor-panel';
      } else if (user.role === 'ROLE_ADMIN' || user.userType === 'admin') {
  ...
  }, [isAuthenticated, user, registrationStep, router]);
